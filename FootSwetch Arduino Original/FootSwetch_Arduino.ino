...
//===================================INICIALIZAÇÃO TESTES LEDS================================//
void testesLed(){
  int led[5] = {led1, led2, led3, led4, led5};
  int rel[5] = {rel1, rel2, rel3, rel4, rel5};
  for (int i = 0; i < 5; i++) {
    digitalWrite(led[i], HIGH);   //APAGA LEDS EM MODO LOOP
    digitalWrite(rel[i], HIGH);   //APAGA LEDS EM RELÉ
    delay(200); 
  }
    for (int i = 0; i < 5; i++) {
    digitalWrite(led[i], LOW);   //APAGA LEDS EM MODO LOOP
    digitalWrite(rel[i], LOW);   //APAGA LEDS EM RELÉ
    delay(200); 
  }
...
//===================================APAGAR BANCO================================//
void clearBanq()  {                             //PISCA LED PARA CONFIRMAÇÃO DE SALVAMENTO EM MEMÓRIA
  int cont = 0; 
 for (int i = 0; i < 5; i++) {
          digitalWrite(led1, HIGH);
          digitalWrite(led2, HIGH);
